...
import { BsPlay, BsEye, BsEyeSlash } from "react-icons/bs";
...
export default function LayerItem({
  ...
  const wrapperCls = `layer-item flex flex-col ${(!visible || isDragging) && 'opacity-50'} `;
  ...
  return (
    <div className={wrapperCls}>
      ...
      >
        <div
          className={`flex items-center p-1 pr-2 border-b gap-1' ${level === 0 && 'border-t'} ${selected && 'bg-indigo-200'}`}
        >
          <div
            style={{ marginLeft: `${level * 10}px` }}
            className={`cursor-pointer ${!components.length && 'pointer-events-none opacity-0'}`}
            onClick={toggleOpen}
          >
            <BsPlay className={`${open ? 'rotate-90' : ''}`} />
          </div>
          <div className="truncate flex-grow" style={itemStyle}>
            {name}
          </div>
          <div
            className={`group-hover:opacity-100 cursor-pointer' ${visible ? 'opacity-0' : 'opacity-100'}`}
      ...
      {!!(open && components.length) && (
        <div className={`w-full ${!open && 'hidden'}`}>{cmpToRender}</div>
      )}
    </div>
  );
}
